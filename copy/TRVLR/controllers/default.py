...
@auth.requires_login()
def maketrip():
    form=FORM('Location', INPUT(_place='place'), 'Date from', INPUT(_datefrom='datefrom'), 'Date to', INPUT(_dateto='dateto'),INPUT(_type='submit'))
    return dict(form=form)
...
def users():
    ...
    if request.args(0) is not None:
        ...
        context = dict(name=name,followers=followers,following=following,picture=picture,description=description,
            experience=experience,gender=gender, #it_name=it_name, 
